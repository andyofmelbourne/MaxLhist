...
\label{error}
\end{figure}
Now let's look at the error as a function of $\hat{\mu}_1^r$ in Fig. \ref{error}. The good thing is that the error seems nice and quadratic near the correct value of $\hat{\mu}_1^r$ (-40) however we can also see that the error levels out at larger or lower values. This is due to rounding errors in the natural logarithm, when evaluating the error metric. Basically, if our initial guess for the $\mu$ values is inside the flat region then we are screwed.
